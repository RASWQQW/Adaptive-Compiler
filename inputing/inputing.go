...
type Compiller struct {
	header string // it just gives us val as self.header
	topic  string
	code   string
}

func (cmp *Compiller) cppCompiler() string {
	...
	GetTaskId, taskType, _ := gb.GetTaskByName(cmp.topic)
	rep("Function name: ", cmp.topic)
	rep("User Code: ", cmp.code)
	...
	if taskType == "PT" {
		if strings.Contains(cmp.code, "#include iostream") || strings.Contains(cmp.code, "using namespace std;") {
			// here goes code only whcih sent with custom and untemplated input and with other values
			// executes with regex

			if strings.Contains(strings.ReplaceAll(cmp.code, " ", ""), "cin>>") {
			...
			if strings.Contains(cmp.code, CurrentFuncName) && cmp.code[:5] == "void" {
				...
				var Inputs string = cmp.code[:len("Arg Count")]
		...
		if strings.Contains(cmp.code, strings.ReplaceAll(CurrentFuncName, " ", "")) {
			...
			var substr = strings.ReplaceAll(cmp.code[0:strings.Index(cmp.code, coType)], " ", "")
...
func (cmp *Compiller) javaCompiler() string {
	return ""
}

func (cmp *Compiller) pythonCompiler() string {
	return ""
}

func (cmp *Compiller) csharpCompiler() string {
	return ""
}
...
func Main(lang string, code string, topic string) string {
	...
	if lang == "cpp" {
		return Mainer.cppCompiler()
	} else if lang == "java" {
		return Mainer.javaCompiler()
	} else if lang == "csharp" {
		return Mainer.csharpCompiler()
	} else if lang == "python" {
		return Mainer.pythonCompiler()
	}
